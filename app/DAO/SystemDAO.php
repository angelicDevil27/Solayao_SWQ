...
class SystemDAO extends BaseDAO{
	...
	//-------Quotes--------//
	  ...
	  function view_Quotes(){
	  	$this->openConn();
	  		...
	  		while($row = $stmt->fetch()){
	  			echo "<tr id=".$row[0].">";
	  			echo "<td>".$row[1]."</td>";
	  			echo "<td><img class='icon-trash' onclick='delete_Quotes(".$row[0].")'/>";
	  			echo "<img class='icon-edit' onclick='edit_Quotes(".$row[0].")'/></td>";
	  			echo "</tr>";	  				  				  			
	  		}
	  ...
	  function display_front_quotes() {
	   ...
	   function post_Quotes($quotesText, $quotes_comment){
	  	$this->openConn();
	  		...
	  		echo "<tr id=".$id.">";
	  			...
	  			echo "<td><img class='icon-trash' onclick='delete_Quotes(".$id.")'/>";
	  			echo "<img class='icon-edit' onclick='edit_Quotes(".$id.")'/></td>";
	  		echo "</tr>";
	  ...
	  function edit_Quotes($quotes_id){
	  	$this->openConn();
	  		...
	  		$stmt->bindParam(1, $quotes_id);
			$stmt->execute();
		
			      $record = $stmt->fetch();
			
			      $content = array("quotes_id"=>$record[0], "quotesText"=>$record[1]);
	  ...
	  function likePost($Addlike,$count_like){
			
			$this->openConn();
			
			$stmt2 = $this->dbh->prepare("SELECT COUNT(likeID) FROM quotes");
			$stmt2->bindParam(1, $count_like;
			$stmt->excute();
			
			
			$stmt = $this->dbh->prepare("INSERT INTO quotes(likeID) VALUES (?)");
			$stmt->bindParam(1, $Addlike);
			$stmt->execute();
			$id = $this->dbh->lastInsertId();
	 ...
	 */	
	  ...
	  function view_comment($comment_id){
	  	$this->openConn();
...
and u.id= ?;");
      ...
      function deleteContacts($id,$myusername){
			      ...
			      $stmt = $this->dbh->prepare("DELETE FROM contact WHERE contact.id = ?");
			      $stmt->bindParam(1, $id);
			      $stmt->execute();
      ...
      function searchContacts($lastname,$myusername){
		      ...
		      $stmt = $this ->dbh->prepare("SELECT * FROM contact where lname like '".$lastname."%'?");
